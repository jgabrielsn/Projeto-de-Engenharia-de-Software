...
describe('CRUD Contas', () => {
    it ('Create a new conta', async () => {
        const response = await request('http://localhost:3333').post('/contas').send({
            UserID: "13",
            contaNome: "teste",
            valor: "100.50",
            vencimento: "01/01/24",
    ...
    it ('Get all contas', async () => {
        const response = await request('http://localhost:3333').get('/contas');
        expect(response.statusCode).toBe(200);
    });
    it ('Get conta by user ID', async () => {
        const response = await request('http://localhost:3333').get('/contas/13');
        expect(response.statusCode).toBe(200);
    });
    it ('Get conta by user ID with invalid ID', async () => {
        const response = await request('http://localhost:3333').get('/contas/1');
        expect(response.statusCode).toBe(400);
    });
    ...
    it ('Update conta', async () => {
        const response = await request('http://localhost:3333').put('/contas/10').send({
            UserID: "13",
            contaNome: "teste",
            valor: "100.50",
            vencimento: "01/01/24",
    ...
    it ('Update conta with a user that does not exist', async () => {
        const response = await request('http://localhost:3333').put('/contas/1').send({
            UserID: "1",
            contaNome: "teste",
            valor: "100.50",
            vencimento: "01/01/24",
    ...
    it ('Delete conta', async () => {
        const response = await request('http://localhost:3333').delete('/contas/10');
        expect(response.statusCode).toBe(204);
    });
    it ('Delete conta with invalid ID', async () => {
        const response = await request('http://localhost:3333').delete('/contas/1');
        expect(response.statusCode).toBe(400);
    });
});
